...
public class UrbanDecay extends PApplet {
	...
	public void setup() {
		 ...
		 */
		/*
		 * try { kinectReader = new KinectBodyDataProvider("test.kinect", 10); }
		 * catch (IOException e) {
	...
	public void draw() {
		...
		if (person != null) {
			...
			// if(!field[i].clicked){
			// field[i].update(field[i].location.x+head.x,
			// field[i].location.y+head.y);
			// field[i].draw();
			// field[i].update(field[i].location.x-head.x,
			// field[i].location.y-head.y);
			...
			// stroke(255,0,0, 100);
			// noFill();
			// strokeWeight(.05f); // because of scale weight needs to be much
			// thinner
			// curve(
